...
SocketClient.prototype._fetchData = function(zoo, buffer, method) {
    ...
    client.on('error', function(err) {
        client.destroy();
        return q.reject(ERROR['105'] + (err.message || err));
    });
    ...
    client.on('close', function(err) {
        ...
        if (err) {
            return q.reject(ERROR['110'] + (err.message || ''));
        }
        // 106 service response error
        if (heap[3] !== 20) {
            ret = heap.slice(18, heap.length - 1).toString();
            return q.reject(ERROR['106'] + ret);
        }
        ...
        if (_ret instanceof Error || offset === 18) {
            return q.reject(ERROR['108'] + (_ret.message || _ret));
        }
        q.resolve(_ret);
    });
